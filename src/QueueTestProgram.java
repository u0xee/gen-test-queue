...
public class QueueTestProgram {
    ...
    public static void printFailedTests(int test_count,
	    ...
	    for (Map.Entry<String, Object> entry : fail.entrySet()) {
		String key = entry.getKey();
                Object val = entry.getValue();
		if (val instanceof List) {
		    List val_list = (List) val;
		    String v = val_list.size() + " " + (key.equals("script") ? "" : val.toString());
    ...
    public static List<Map<String, Object>> runTests(int test_count, int returned_failures_count,
        ...
        PriorityQueue<Map<String, Object>> failing =
                new PriorityQueue<>(10 /*capacity hint*/,
				    ...
				    Integer fill = (Integer) action.get(1);
                                    script_weight += 6 + fill / 8; // avoid meta-actions if possible
				}
		...
		System.out.println("Found a failing case! Shrinking...");
		...
		System.out.println("  Done shrinking.");
                ...
                if (failing.size() > returned_failures_count * 1) break;
            }
        }
...
class Shrink {
    ...
    shrinkNumber(List<List<Object>> script, List<Object> action,
                 Function<List<List<Object>>, Boolean> eval_failed) {
        Integer num = (Integer) action.get(1);
	for (int i = 0; i < num; i++) {
	    action.set(1, i);
	    if (eval_failed.apply(script)) {
		return true;
	    }
	}
	action.set(1, num);
    ...
    public static boolean
    shrinkNumbersHelper(List<List<Object>> script,
                        Function<List<List<Object>>, Boolean> eval_failed) {
	boolean shrunk = false;
	for (int i = 0; i < script.size(); i++) {
	    List<Object> action = script.get(i);
	    if (action.size() > 1 && action.get(1) instanceof Integer) {
		if (shrinkNumber(script, action, eval_failed)) {
		    shrunk = true;
		}
	    }
	}
    ...
    public static boolean
    shrinkNumbers(List<List<Object>> script,
                  Function<List<List<Object>>, Boolean> eval_failed) {
	boolean shrunk = false;
	while (shrinkNumbersHelper(script, eval_failed)) {
	    shrunk = true;
	}
	return shrunk;
    }
    ...
    public static List<List<Object>> deepCopy(List<List<Object>> script) {
	return script.stream().map((action) -> new ArrayList<>(action))
	    .collect(Collectors.toCollection(ArrayList::new));
    }
	...
	} while(shrinkNumbers(script, eval_failed));
